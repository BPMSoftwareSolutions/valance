...
export async function evaluate(content, rule, context) {
  try {
    ...
    // Check if this is actually a plugin directory (has manifest.json)
    ...
    try {
      ...
      return {
        passed: true,
        message: "Skipped - not a plugin directory (no manifest.json)",
      };
    }

    // Read related files
    const sequencePath = path.join(pluginDir, "sequence.js");
    ...
    if (rule.checkManifestConsistency && manifest && sequence) {
      const manifestValidation = validateManifestConsistency(
        manifest,
        sequence
      );
...
function validateManifestConsistency(manifest, sequence) {
  ...
  if (manifest.id !== sequence.id) {
    errors.push(
      `Manifest ID '${manifest.id}' doesn't match sequence ID '${sequence.id}'`
    );
  }
  ...
  if (manifest.version !== sequence.version) {
    errors.push(
      `Manifest version '${manifest.version}' doesn't match sequence version '${sequence.version}'`
    );
  }
  ...
  if (!manifest.entry) {
    ...
    warnings.push(
      `Manifest entry '${manifest.entry}' should typically be 'index.ts' or 'index.js'`
    );
  }
...
function validateSequenceRegistration(indexContent) {
  ...
  const registryPattern =
    /import\s+.*registerSequence.*from\s+['"]@\/registry['"]/;
...
function validateImportStatements(content, sequencePath) {
  ...
  const sequenceImportMatch = content.match(
  ...
  if (sequenceImportMatch) {
    ...
    if (imports !== "sequence") {
      warnings.push(
        `Sequence import should be '{ sequence }', found '{ ${imports} }'`
      );
    }
  }
  ...
  return {
    passed: errors.length === 0,
    message:
      errors.length === 0 ? "Import statements valid" : errors.join(", "),
