...
const pluginInfo = {
  ...
  version: "1.0.0",
  description:
    "Main application shell and layout management for RX.Evolution.client",
  author: "RenderX Team",
  type: "symphony",
...
var CIAPlugin = {
  mount: (conductor, eventBus) => {
    console.log("🎼 App Shell Symphony: Mounting via CIA interface...");
    ...
    try {
      ...
      console.error("🚨 App Shell Symphony: Mount failed:", error);
  ...
  unmount: (conductor, eventBus) => {
    console.log("🎼 App Shell Symphony: Unmounting via CIA interface...");
    ...
    try {
      ...
      console.error("🚨 App Shell Symphony: Unmount failed:", error);
...
if (typeof window !== "undefined" && window.RenderX?.debug) {
  if (
    ...
    !import_sequence.sequence.id ||
    !import_sequence.sequence.movements
  ) {
