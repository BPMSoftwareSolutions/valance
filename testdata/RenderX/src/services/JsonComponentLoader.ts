...
export class JsonComponentLoader {
  ...
  async loadAllComponentsMusical(): Promise<JsonComponentLoadResult> {
    if (!this.conductor) {
      console.warn('🚨 JsonComponentLoader: No conductor connected, falling back to direct loading');
    ...
    console.log(`🎼 JsonComponentLoader: Started musical loading sequence ${sequenceId} for ${componentFiles.length} components`);
  ...
  private async performLoad(): Promise<JsonComponentLoadResult> {
    ...
    try {
      ...
      console.log(`🔍 Found ${componentFiles.length} JSON component files to load`);
      ...
      for (const filename of componentFiles) {
        try {
          ...
          console.log(`✅ Loaded JSON component: ${component.metadata.name} (${filename})`);
        } catch (error) {
          const errorMessage = error instanceof Error ? error.message : 'Unknown error';
          result.failed.push({ filename, error: errorMessage });
          console.error(`❌ Failed to load JSON component ${filename}:`, errorMessage);
        }
      }
      ...
      console.log(`🎼 JSON Component loading complete: ${result.success.length} loaded, ${result.failed.length} failed`);
  ...
  private async getComponentFiles(): Promise<string[]> {
    console.log('🔍 JsonComponentLoader: Starting dynamic component discovery...');
    ...
    try {
      ...
      if (indexResponse.ok) {
        ...
        if (Array.isArray(indexData.components)) {
          console.log('📋 Found components index file with', indexData.components.length, 'components');
          discoveredFiles.push(...indexData.components);
        }
      }
    } catch (error) {
      console.log('📋 No components index file found, using discovery strategies');
    }
    ...
    if (discoveredFiles.length === 0) {
      ...
      console.log('🔍 Attempting to discover components by trying common names...');
    ...
    for (const filename of discoveredFiles) {
      try {
        ...
        if (response.ok) {
          ...
          console.warn(`⚠️ Component file ${filename} listed but not accessible`);
        }
      } catch (error) {
        console.warn(`⚠️ Component file ${filename} verification failed:`, error);
      }
    }

    console.log(`🎯 Component discovery complete: ${verifiedFiles.length} components found`);
